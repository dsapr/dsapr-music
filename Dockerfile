...
# 执行代码编译命令
RUN mvn -f /app/pom.xml -DskipTests=true -Dspring.profiles.active=prod clean package
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/dsapr-music-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/dsapr-music-0.0.1.jar"]